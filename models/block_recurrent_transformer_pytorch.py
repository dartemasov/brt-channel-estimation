...
class Attend(nn.Module):
    ...
    def flash_attn(self, q, k, v, mask = None):
        ...
        # TODO: check and validate the applciation of sdp kernels
...
class BlockRecurrentTransformer(nn.Module):
    def __init__(
        ...
        max_seq_len = 1024,
        block_width = 512,
        recurrent_layers: Optional[list] = None,
        read_recurrent_layers: Optional[list] = None,
        ...
        meas_mat = np.load(meas_mat_path, allow_pickle=True).item()['meas_mat']
        block_line1 = np.hstack((np.real(meas_mat), -np.imag(meas_mat)))
        block_line2 = np.hstack((np.imag(meas_mat), np.real(meas_mat)))
        self.meas_matrix = torch.from_numpy(np.vstack((block_line1, block_line2))).to(device)
        self.W_pinv = torch.linalg.pinv(self.meas_matrix)   # pinv of the measurement matrix
        self.step = self.meas_matrix.shape[1] / torch.trace(torch.mm(self.W_pinv, self.meas_matrix))
        self.W_pinv_mul_step = (self.step * self.W_pinv).to(torch.float)    # pinv of the measurement matrix, multiplied by the step size
        self.depth = n_time_steps

        self.eps = 0
        
        self.to_logits = nn.Sequential(
            nn.Linear(in_features=n_ant,out_features=2*n_ant),
            nn.ReLU(),
            nn.Linear(in_features=2*n_ant,out_features=n_ant))
        
        self.res_weights = torch.nn.Parameter(torch.linspace(1,0.3,n_time_steps))
    ...
    def name(self):
        ...
        '''
        return 'RecurrentTransformerEstimator'
    

    def forward(self, x, xl_memories: List[torch.Tensor] = [], states: List[torch.Tensor] = [], return_memories_and_states = None):
        x = (self.W_pinv_mul_step @ x[...,None]).squeeze(-1)
        # x = h_tilde[:,None,:]
        ...
        for step in range(self.n_time_steps):
            ...
            for idx, (attn, ff) in enumerate(self.layers):
                attn_kwargs = dict(
                    ...
                    read_from_state_containers = self.read_state_router[idx+1]
                )
                # attention layer
            ...
            rw = torch.clamp(self.res_weights[step], min=0.1)
            x = x - rw*input_block
            states = next_states

            if input_block_length == self.block_width:
